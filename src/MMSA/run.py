...
def MMSA_run(
    ...
    if model_save_dir == "": # use default model save dir
        model_save_dir = os.path.join("home","WorkSpace","MMSA","saved_models")
    Path(model_save_dir).mkdir(parents=True, exist_ok=True)
    if res_save_dir == "": # use default result save dir
        #res_save_dir = Path.home() / "MMSA" / "results"
        res_save_dir = os.path.join("home","WorkSpace","MMSA","results")
    Path(res_save_dir).mkdir(parents=True, exist_ok=True)
    if log_dir == "": # use default log save dir
        #log_dir = Path.home() / "MMSA" / "logs"
        log_dir = os.path.join("home","WorkSpace","MMSA","logs", f'{model_name}-{time_now}')
    ...
    if is_tune: # run tune
        ...
        for i, seed in enumerate(seeds):
            ...
            # actual running
            args['model_save_path'] = Path(log_dir) / f"{args['model_name']}-{args['dataset_name']}-{'model'}-{str(seed)}.pth"
            args['optim_save_path'] = Path(log_dir) / f"{args['model_name']}-{args['dataset_name']}-{'optim'}-{str(seed)}.pth"
            result = _run(args, num_workers, is_tune, log_dir)
            logger.info(f"Result for seed {seed}: {result}")
            model_results.append(result)
        criterions = list(model_results[0].keys())
        # save result to csv
        csv_file = res_save_dir / f"{dataset_name}_{model_name}_{time_now}.csv"
        if csv_file.is_file():
            df = pd.read_csv(csv_file)
        else:
...
def _run(args, num_workers=4, is_tune=False, log_dir=None, from_sena=False):
    # load data and models
    dataloader = MMDataLoader(args, num_workers)
    model = AMIO(args).to(args['device'])

    # -------------------------------------------------
    torch_summary = SummaryWriter(log_dir)
    ...
    epoch_results = trainer.do_train(model, dataloader, torch_summary, return_epoch_results=from_sena)
    ...
    if from_sena:
        ...
        results, _ = trainer.do_test(model, dataloader['test'], mode="TEST")
        # delete saved model
        Path(args['model_save_path']).unlink(missing_ok=True)
    else:
        results,_= trainer.do_test(model, dataloader['test'], torch_summary, mode="TEST")
